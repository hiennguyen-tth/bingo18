...
Crawler tự chạy mỗi **60 giây** trong giờ mở thưởng (06:00–22:00 VN). **Startup recovery** crawl 5 trang khi boot. **Periodic deep recovery** phát hiện gap trong 30 ky gần nhất và crawl thêm 3 trang (max 1 lần/10 phút). `crawl.js` dùng timeout ngắn + no-cache headers + fallback source để giảm lag nhưng vẫn giữ rate-limit lịch sự.

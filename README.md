...
**v12.4 Changes (current):**
- **AJAX-only crawler:** Bỏ hoàn toàn HTML page crawl. HTML `xs-bingo-18.html` nặng ~122KB, 700ms, bị server-cache nhiều phút. AJAX endpoint `GetKetQuaBinGo18More` chỉ 15KB, ~200ms, trả data real-time. Không còn `fetchHtml()`, không còn `Promise.all` merge — 1 request/tick đơn giản và nhanh hơn.
- **Crawl interval 30s → 12s:** AJAX nhẹ đủ để poll 12s an toàn. Max lag giảm từ 30s xuống 12s. Draws mỗi ~6 phút → 30× margin.
...
| **Crawl lag 2-3 kỳ** | HTML page `xs-bingo-18.html` có server-side origin cache. `?_t=timestamp` chỉ bypass CDN, không bypass origin → HTML luôn stale; AJAX (real-time) chỉ được gọi khi HTML *fail hoàn toàn* | **Parallel fetch:** HTML + AJAX đồng thời (`Promise.all`) → merge unique records → luôn lấy ky mới nhất từ cả hai nguồn. Interval 60s → **30s** để giảm max wait thêm 30s. |
...
Crawler tự chạy mỗi **30 giây** trong giờ mở thưởng (06:00–22:00 VN). Mỗi tick fetch **HTML và AJAX song song** (`Promise.all`) rồi merge unique records — đảm bảo luôn lấy ky mới nhất dù HTML page có server-side cache. **Startup recovery** crawl 5 trang khi boot. **Periodic deep recovery** phát hiện gap trong 30 ky gần nhất và crawl thêm 3 trang (max 1 lần/10 phút).

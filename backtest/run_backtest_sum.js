 ...
 *
 * For each record i >= WINDOW, uses predictSum on data[0..i-1]
 * and checks if the actual sum matches top-1, top-3, or top-5.
 *
 * Random baselines: 1/16 = 6.25%, 3/16 = 18.75%, 5/16 = 31.25%
 *
 * Usage:  node backtest/run_backtest_sum.js
 */
...
async function runBacktestSum() {
  ...
  for (let i = START_I; i < N; i += SAMPLE_EVERY) {
    ...
    const result = predict.predictSum(slice)

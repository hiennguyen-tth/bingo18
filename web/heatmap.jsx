 ...
 */
window.Heatmap = function Heatmap({ history }) {
  const { useMemo } = React
  ...
  const S = {
    ...
    cell: {
      ...
      textAlign: 'center', fontSize: 11, color: '#e2e8f0',
  ...
  return (
    <div>
      <div style={S.title}>Number Position Heatmap</div>
      <div style={S.grid}>
        <div />
        {[1, 2, 3, 4, 5, 6].map(n => (
          <div key={n} style={S.colHead}>{n}</div>
        ))}
        {['n1', 'n2', 'n3'].map(pos => (
          <React.Fragment key={pos}>
            <div style={S.rowLabel}>{pos.toUpperCase()}</div>
            {[1, 2, 3, 4, 5, 6].map(n => {
              const v = data.counts[`${pos}-${n}`] || 0
              return (
                <div
                  key={n}
                  style={{ ...S.cell, background: cellBg(v) }}
